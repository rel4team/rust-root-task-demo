use async_runtime::{coroutine_wake, utils::IndexAllocator, CoroutineId};
...
pub struct Socket {
    ...
    pub buffers: Option<&'static mut TcpBuffer>, // datas
    pub read_ready_len: usize,
    pub seq: u32,
    pub ack: u32,
    pub block_task: Option<CoroutineId>,
}


static mut TCP_BUFFER_POOL: [TcpBuffer; 128] = [TcpBuffer::new(); 128];
static mut BUFFER_ALLOCATOR: IndexAllocator<128> = IndexAllocator::new();
...
pub fn push_data(index: usize, packet: &TCPPacket) {
    ...
    if socket.buffers.is_none() {
        unsafe {
            let index = BUFFER_ALLOCATOR.allocate().unwrap();
            let buffer = &mut TCP_BUFFER_POOL[index];
            socket.buffers.replace(buffer);
        }
    }
    socket.buffers.as_mut().unwrap().data[..packet.data_len].copy_from_slice(packet.data);
    socket.read_ready_len = packet.data_len;
    match socket.block_task.take() {
        Some(cid) => {
            // debug_println!("wake read task");
            coroutine_wake(&cid);
        }
        _ => {
