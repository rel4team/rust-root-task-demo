...
use sel4::{
    get_clock, CNode, CapRights, LocalCPtr, ObjectBlueprint, ObjectBlueprintArch, VMAttributes, TCB,
};
...
use crate::async_lib::{
    recv_reply_coroutine, register_async_syscall_buffer, register_recv_cid, uintr_handler,
};
...
pub fn async_syscall_test(bootinfo: &sel4::BootInfo) -> sel4::Result<!> {
    ...
    // };
    //生成接收通知的ntfn
    let reply_ntfn = obj_allocator.lock().alloc_ntfn().unwrap();
    //拿tcb
    let recv_tcb = sel4::BootInfo::init_thread_tcb();
    //tcb绑定ntfn
    recv_tcb.tcb_bind_notification(reply_ntfn)?;
    //taic
    let process_id = alloc_receiver(recv_tcb, reply_ntfn, 0);
    //生成buffer
    let ipc_new_buffer = unsafe { obj_allocator.lock().alloc_new_buffer_without_free() };
    let new_buffer_ptr = ipc_new_buffer as *mut NewBuffer as usize;
    ...
    //生成一个接收回复的ntfn的capability,该capability无标记
    //拿一个空槽，用于复制出一个新的capability并赋予标记
    ...
    debug_println!("async_syscall_test: process_id: {:?}", process_id.unwrap());
    reply_ntfn.register_async_syscall(new_buffer_cap)?;
    ...
    //注册接收者 ——>（client_process_id给他发，由cid协程进行处理）
    // register_receiver(63, 1 as usize);
    //注册发送者 ——>谁来发（谁接收回复）
...
async fn test_async_output_section(vaddr: usize) {
    ...
    debug_println!(
        "test_async_riscvpage_get_address: sync RISCVPageGetAddress get paddr: {:#x}",
        paddr
    );
}
...
async fn test_async_notification_section(obj_allocator: &Mutex<ObjectAllocator>) {
    ...
    let target_tcb_bits = obj_allocator
        .lock()
        .create_thread(test_helper_thread, async_args.get_ptr(), 255, 1, true)
        .unwrap()
        .cptr()
        .bits();
    ...
    .await;
    let notification = sel4::BootInfo::init_cspace_local_cptr::<sel4::cap_type::Notification>(slot);
    // 绑定Notification
...
fn test_sync_riscv_page_map(obj_allocator: &Mutex<ObjectAllocator>) {
    ...
    data.change_data();
    debug_println!(
        "test_async_riscv_page_unmap: unmap success, data value: {:?}",
        data.data
    );
}
...
async fn test_async_riscv_page_section(obj_allocator: &Mutex<ObjectAllocator>) {
    ...
    let pt_untyped = obj_allocator
        .lock()
        .get_the_first_untyped_slot(&pt_blueprint);
    ...
    .await;
    let page_table = sel4::BootInfo::init_cspace_local_cptr::<sel4::cap_type::_4KPage>(pt_slot);
    ...
    let frame_untyped = obj_allocator
        .lock()
        .get_the_first_untyped_slot(&frame_blueprint);
